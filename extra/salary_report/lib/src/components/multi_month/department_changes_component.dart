...
class DepartmentChangesComponent extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return departmentChangesState.when(
      data: (departmentChanges) {
        ...
        employeeChangesMap.forEach((month, changes) {
          if (changes['newEmployees']!.isNotEmpty ||
              changes['resignedEmployees']!.isNotEmpty) {
            hasChanges = true;
          }
        });
        departmentEmployeeChangesMap.forEach((month, deptChanges) {
  ...
  Map<String, Map<String, List<MinimalEmployeeInfo>>> _calculateEmployeeChanges(
    ...
    for (int i = 0; i < sortedMonths.length; i++) {
      ...
      if (!employeeChanges.containsKey(monthKey)) {
        employeeChanges[monthKey] = {
          'newEmployees': <MinimalEmployeeInfo>[],
          'resignedEmployees': <MinimalEmployeeInfo>[],
        };
      }

      // 如果不是第一个月份，比较与前一个月的员工变化
      if (i > 0) {
        final previousMonth = sortedMonths[i - 1];
        final currentWorkers = currentMonth.workers;
        final previousWorkers = previousMonth.workers;
        ...
        employeeChanges[monthKey]!['newEmployees'] = newEmployees;
        employeeChanges[monthKey]!['resignedEmployees'] = resignedEmployees;
      }
    }
  ...
  _calculateDepartmentEmployeeChanges(
    ...
    // 遍历每个月份，比较与前一个月的员工变化（按部门分组）
    for (int i = 0; i < sortedMonths.length; i++) {
      final currentMonth = sortedMonths[i];
      final monthKey =
          '${currentMonth.year}-${currentMonth.month.toString().padLeft(2, '0')}';

      if (!departmentEmployeeChanges.containsKey(monthKey)) {
        departmentEmployeeChanges[monthKey] = {};
      }
      ...
      if (i > 0) {
        ...
        // 分组当前月员工
        for (var worker in currentMonth.workers) {
        ...
        // 分组前一个月员工
        for (var worker in previousMonth.workers) {
        ...
        for (var department in allDepartments) {
          if (!departmentEmployeeChanges[monthKey]!.containsKey(department)) {
            departmentEmployeeChanges[monthKey]![department] = {
              'newEmployees': <MinimalEmployeeInfo>[],
              'resignedEmployees': <MinimalEmployeeInfo>[],
            };
          }
          ...
          departmentEmployeeChanges[monthKey]![department]!['newEmployees'] =
              newEmployees;
          departmentEmployeeChanges[monthKey]![department]!['resignedEmployees'] =
              resignedEmployees;
        }
      }
    }
