// src/services/yearly/docx_writer_service.dart
...
class YearlyDocxWriterService {
  /// 写入年度报告
  Future<String> writeReport({
    required YearlyReportContentModel data,
    required YearlyReportChartImages images,
  }) async {
    // 使用年度报告模板
    final templatePath = 'assets/salary_report_template_annual.docx';

    logger.info('开始写入年度报告, 使用模板: $templatePath');
  ...
  /// 生成报告文件名
  String _generateReportName(YearlyReportContentModel data, String timestamp) {
    final companyName = data.companyName;
    final reportTime = data.reportTime;
    final typeDescription = '年度';
  ...
  Content _buildContent(
    YearlyReportContentModel data,
    YearlyReportChartImages images,
  ) {
    ...
    // 添加年度报告特定字段
    _addYearlySpecificFields(content, data);
  ...
  /// 添加通用文本字段
  void _addCommonTextFields(Content content, YearlyReportContentModel data) {
    content
  ...
  /// 添加表格内容
  void _addTableContent(Content content, YearlyReportContentModel data) {
  ...
  /// 添加图片内容
  void _addImageContent(Content content, YearlyReportChartImages images) {
